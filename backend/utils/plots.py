...
class Annotator:
    ...
    def text(self, xy, text, txt_color=(255, 255, 255)):
        ...
        bbox = self.font.getbbox(text)  # get text bounding box
...
def output_to_target(output):
    ...
    for i, o in enumerate(output):
        targets.extend([i, cls, *list(*xyxy2xywh(np.array(box)[None])), conf]
...
def save_one_box(xyxy, im, file=Path('im.jpg'), gain=1.02, pad=10, square=False, BGR=False, save=True):
    ...
    crop = im[int(xyxy[0, 1]):int(xyxy[0, 3]), int(xyxy[0, 0])
                  :int(xyxy[0, 2]), ::(1 if BGR else -1)]
    if save:
